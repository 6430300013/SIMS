...
<body class="bg-light">
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        empty($_POST["id"]) ? $error["id"] = "*กรุณาระบุรหัสนักศึกษา" : $id = protectInput($_POST["id"]);
        if(!empty($_POST["email"])){
            !filter_var($_POST["email"], FILTER_VALIDATE_EMAIL) ? $error["email"] = "*กรุณาระบุอีเมลให้ถูกต้อง" : $email = protectInput($_POST["email"]);
        }
        empty($_POST["en_name"]) ? $error["en_name"] = "*กรุณาระบุชื่อ" : $en_name = protectInput($_POST["en_name"]);
        empty($_POST["en_surname"]) ? $error["en_surname"] = "*กรุณาระบุนามสกุล" : $en_surname = protectInput($_POST["en_surname"]);
        empty($_POST["th_name"]) ? $error["th_name"] = "*กรุณาระบุชื่อ" : $th_name = protectInput($_POST["th_name"]);
        empty($_POST["th_surname"]) ? $error["th_surname"] = "*กรุณาระบุนามสกุล" : $th_surname = protectInput($_POST["th_surname"]);
        if(!empty($_POST["email"])){
        ...
        if(empty($error["email"]) && empty($error["major_code"]) && !empty($_POST["id"]) && !empty($_POST["en_name"]) && !empty($_POST["en_surname"]) && !empty($_POST["th_name"]) && !empty($_POST["th_surname"])){
            ...
            if(!$query){
                echo '<script> Swal.fire("เพิ่มข้อมูลล้มเหลว") </script>';
            }else{
                header("Location: ./student.php?status=2");
                exit();
            }
        }else{
            echo '<script> Swal.fire("กรุณาระบุข้อมูลให้ถูกต้อง") </script>';
        }
    }
    ?>
    <div class="container d-flex flex-column justify-content-center align-items-center">
        <div >
            <div class="all-header "><h1>Insert</h1></div>
            <div>
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" class="row">
                    ...
                    <div>
                    <button type="submit" class="btn bt-y">Save</button>
                    <a href="./student.php" class="btn bt-b">Back</a>
                    </div>
                </form>
            </div>
        </div>
